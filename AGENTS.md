...
- `./build.sh` — configure and build Debug firmware, then create ELF, HEX, and BIN artifacts under `build/Debug/`.
- `./build.sh Release --clean` — perform a fresh size-optimized build.
- `./flash.sh` — rebuild, then program and verify through ST-Link/OpenOCD.
- `./flash.sh --no-build --dry-run` — validate the flash command without accessing hardware.
