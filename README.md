...
基于 STM32F103RCT6 的课程设计多波形信号发生器。项目计划使用片内 12 位 DAC、定时器和 DMA 输出正弦波、方波、三角波及锯齿波，并通过 OLED、两个板载按键和四个外接按键完成显示与控制。

当前状态：第一版固件已经完成并通过 Debug/Release 交叉编译，包含四波形 DAC DMA 输出、板载双键与外接四键控制、OLED 状态页和串口日志；开发板尚未到货，输出质量和烧录流程等待实测。
...
| 输入 | 两个板载按键、四个外接常开按键 | 板载键提供启停和波形切换，外接四键功能保持不变 |
...
| 板载 K3 启停/K4 切换波形 | PC8/PC9 | GPIO Input，Pull-up |
...
上电后默认以 1 kHz 输出正弦波。板载 K3（PC8）和外接 PB12 均控制启停，板载 K4（PC9）和外接 PB13 均按正弦波、方波、三角波、锯齿波循环切换；外接 PB14/PB15 继续降低或提高频率，长按后自动连发。调节步长随频段在 1、10、100 和 1000 Hz 间变化，范围限制为 1 Hz～10 kHz。
